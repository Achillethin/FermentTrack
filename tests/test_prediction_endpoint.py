...
@pytest.mark.asyncio
async def test_prediction_with_a_custom_organism_attached(
    client: AsyncClient, db_session: AsyncSession
) -> None:
    # A custom attachment ADDS to the type's defaults (the biochemistry resolver returns
    # (organism, source, notes)); the forecast must keep both and mark the set custom.
    batch = await _seed(db_session)
    extra = Organism(name="Leuconostoc mesenteroides", kingdom="bacteria", source_version="v1")
    db_session.add(extra)
    await db_session.commit()
    resp = await client.post(
        f"/batches/{batch.id}/organisms", json={"organism_id": str(extra.id), "notes": "starter"}
    )
    assert resp.status_code == 201
    ...
    assert resp.status_code == 200
    body = resp.json()
    assert [o["name"] for o in body["organisms"]] == [
