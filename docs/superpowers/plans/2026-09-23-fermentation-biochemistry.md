...
- **Provenance narrowing:** v1 fetches only canonical identity from KEGG (5 EC entries, 5 compound names); all relations are hand-curated. The three reversal docs say so explicitly.
- **Final review, curation restored:** kombucha now includes Gluconacetobacter xylinus and sourdough uses Lactobacillus sanfranciscensis (as named in the spec); L. sanfranciscensis was added to EC 1.1.1.27 so sourdough keeps lactate. Snapshot regenerated before it was ever migrated anywhere but throwaway SQLite.
- **Known v1 limitations (v2 snapshot candidates, owner's curation call):** garum has no enzymes/compounds (Tetragenococcus halophilus has no ENZYME_ORGANISMS entry); koji/miso have alpha-amylase but no compounds (starch is a KEGG glycan, not a compound, and maltose is not in COMPOUNDS); cheese has lactate dehydrogenase but no protease/rennet ECs (chymosin is an added ingredient, not organism-expressed); Kluyveromyces marxianus has no enzymes; vinegar shows acetaldehyde and acetate but no ethanol (ethanol->acetaldehyde in Acetobacter is PQQ-dependent alcohol dehydrogenase, not in the curated set). A v2 snapshot + migration before the UI increment ships is cheaper than after.
- **Merge order:** this branch was cut from feat/usda-food-catalog at 13fba2a and migration 0008 chains from 0007, which is not on master; feat/usda-food-catalog must merge first.
